...
figure2 <- function(CEX.L =0.9, CEX.A = 1.2){
...
figure3 <- function(CEX.L =1, CEX.A = 0.9){
...
figure4 <- function(CEX.L = 1, CEX.A = 0.9){
par(mfrow=c(1,2), omi=c(0.2, 0.2, 0.2, 0.2), mai=c(0.5, 0.4, 0.5, 0.1), pty="s")
	source("params.r")
	#plot a  - Cost benefit curves over soil water pot
	ENV=seq(Env$psi.soil.sat, -1.5,-0.2)
	fig.setup(c(min(CB_axis), max(CB_axis)), CB_lab, CB_axis, CB_axis2, c(min(SALA_axis), max(SALA_axis)),SALA_lab, SALA_axis, SALA_axis2, CEX.A, CEX.L);
	for(i in 1:length(ENV)){
		Env$psi.soil= ENV[i];
		CB = cost_ben(SALA.vec, RALA, Leaf, Plant, Env)
	...
	#plot b  - SALA optimum vs soil to root resistance
	OPT<-NULL;
	ENV=seq(Env$psi.soil.sat, -1.5,-0.02)
	for(i in 1:length(ENV)){
		Env$psi.soil= ENV[i];
		OP = cost_ben.opt(RALA, Leaf, Plant, Env);
		if(OP[8] > 0)  OPT<-rbind(OPT, c(ENV[i], OP[1], OP[5], OP[3]))
...
figure5 <- function(CEX.L = 0.9, CEX.A = 1.2){
...
figure6 <- function(CEX.L = 0.9, CEX.A = 1.2){
...
figure7 <- function(CEX.L =0.9, CEX.A = 1.2){
...
figure8 <- function(CEX.L =0.9, CEX.A = 1.2){
