...
figure2 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L = 0.9, CEX.A = 1.2, RALA = 3){
  ...
  pars$Env$dw = 20*Ksoil(pars$Env)*RALA; #set dw so that root resistenace = 20
...
figure8 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L = 0.9, CEX.A = 1.2, RALA = 3){
  ...
  for(i in 1:length(ENV)){
    ...
    OP = cost_ben.opt(RALA, pars$Leaf, pars$Plant, pars$Env);		if(OP[8] > 0) 	OPT <- rbind(OPT, c(ENV[i], OP[1], OP[5], OP[3]));
  };	colnames(OPT)=c("ENV", "sala", "leafN", "A")
  #PLOT OPTIMUM
  points(OPT[,1], OPT[,2]*1e4, type='l')
  ...
  Kj = pars$Leaf$kj; Kv = pars$Leaf$kv;
  ...
  OPT <- NULL;
  for(i in 1:length(ENV)){
    pars$Leaf$kj = Kj*ENV[i]; pars$Leaf$kv = Kv*ENV[i];
    OP = cost_ben.opt(RALA, pars$Leaf, pars$Plant, pars$Env);		if(OP[8] > 0)  OPT <- rbind(OPT, c(ENV[i], OP[1], OP[5], OP[3]))
  };	colnames(OPT)=c("ENV", "sala", "leafN", "A")
  #PLOT OPTIMUM
  points(OPT[,1], OPT[,2]*1e4, type='l')
