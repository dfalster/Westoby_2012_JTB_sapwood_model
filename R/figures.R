...
figure2 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L = 0.9, CEX.A = 1.2, RALA = 3){
  ...
  for(i in 1:length(ENV)){
    ...
    if(i==1){ #only one cost line because all the same
      points(CB[1,]*1e4, CB[7,]*1e-3, col=1,  type='l', lty='dashed', cex=cex.cost)
    }
    points(CB[1,]*1e4, CB[6,]*1e-3, col=1,  type='l')  # Revenue
    OP = cost_ben.opt(RALA, pars$Leaf, pars$Plant, pars$Env)

    points(OP[1,]*1e4, OP[6,]*1e-3, col=1,  cex=1, pch=19)  # Optimum
  }
  ...
  for(i in 1:length(ENV)){
    ...
    CB = cost_ben(SALA.vec, RALA, pars$Leaf, pars$Plant, pars$Env)
    if(i==1){  #only one cost line because all the same
          points(CB[1,]*1e4, CB[7,]*1e-3, col=1,  type='l', lty='dashed', cex=cex.cost)  # Invest
      }
    points(CB[1,]*1e4, CB[6,]*1e-3, col=1,  type='l')  # Revenue
    OP = cost_ben.opt(RALA, pars$Leaf, pars$Plant, pars$Env)
  ...
  points(OPT_low[,1], OPT_low[,2]*1e4, type='l')
  points(OPT_high[,1], OPT_high[,2]*1e4, type='l',  lty='dashed')
  ...
  points(OPT_low[,1], OPT_low[,2]*1e4, type='l',  lty='dashed')
  points(OPT_high[,1], OPT_high[,2]*1e4, type='l')
...
figure3 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L = 1, CEX.A = 0.9, RALA = 3){
  ...
  for(i in 1:length(ENV)){
    ...
    CB = cost_ben(SALA.vec, RALA, pars$Leaf, pars$Plant, pars$Env)
    if(i==1){  #only one cost line because all the same
          points(CB[1,]*1e4, CB[7,]*1e-3, col=1,  type='l', cex=cex.cost,  lty='dashed')
      }
    points(CB[1,]*1e4, CB[6,]*1e-3, col=1,  type='l')  # Revenue
    OP = cost_ben.opt(RALA, pars$Leaf, pars$Plant, pars$Env)
  ...
  points(OPT[,1]*1e-3, OPT[,2]*1e4, type='l')
  title('b)', adj=0, cex.main = 1.2)


}

figure4 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L =  1, CEX.A = 0.9, RALA = 3){
  par(mfrow=c(1,2), omi=c(0.2, 0.2, 0.2, 0.2), mai=c(0.5, 0.4, 0.5, 0.1), pty="s")
  pars <- load_default_parameters()
  ...
  #PLOT
  fig.setup(c(min(OPT_axis), max(OPT_axis)), OPT_lab, OPT_axis, OPT_axis2, c(-1.5, 0), "Soil water Potential (MPa)", seq(0, -1.5, -0.25), seq(0, -1.5, -0.125), CEX.A, CEX.L)
  points(OPT[,1], OPT[,2]*1e4, type='l')
  title('b)', adj=0, cex.main = 1.2)


}

figure5 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L =  0.9, CEX.A = 1.2){
  ...
  #PLOT SALA OPTIMUM
  plot(1:2, 1:2, type="n",log="y", axes=F,ann=F, xlim = XLIM, ylim=YLIM, xaxs="i", yaxs="i", las=1)
  	axis(2, at=Y_ax, labels=Y_ax, las=1, tck=0.030, cex.axis=CEX.A, adj = 0.5)
  ...
  axis(4, at=Y_ax, labels=F, las=1, tck=0.030, cex.axis=CEX.A, adj = 0.5)
...
figure6 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L =  0.9, CEX.A = 1.2, RALA = 3){
...
figure7 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L = 0.9, CEX.A = 1.2, RALA = 3){
  ...
  points(OPT2[,1], OPT2[,2]*1e4, type='l')    #plot SALA for otpimising C
  points(OPT1[,1], OPT1[,2]*1e4, type='l',  lty="dashed")     #plot SALA for optimising H
  ...
  XLAB = "VPD  (kPa)"
  X_ax =seq(0.0,2.5, 0.5)
   X_ax2 =seq(0.0, 2.5, 0.25)
  XLIM = c(min(X_ax), max(X_ax))
...
figure8 <- function(SALA.vec =  seq_log(1E-8, 5E-3, 1.01), cex.cost = 1.5, CEX.L = 0.9, CEX.A = 1.2, RALA = 3){
  ...
  mtext(XLAB, side = 1, line = 3.0, outer = F, at= NA, cex=CEX.L)
  mtext(YLAB, side = 2, line = 3.0, outer = F, at= NA, cex=CEX.L)
  #CALCULATE OPTIMUM
  OPT <- NULL
